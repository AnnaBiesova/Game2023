...
namespace _Scripts.Extensions
{
	public static class RectTransformExtensions
	{
        ...
        public static Vector2 GetPixelFromWorldPoint(this RectTransform rectTransform, Vector3 worldPoint)
		{
			...
			if (percentWidth >= 0 && percentWidth <= 1) imagePos = imagePos.SetX(percentWidth * size.x);

			if (percentHeight >= 0 && percentHeight <= 1) imagePos = imagePos.SetY(percentHeight * size.y);

			return imagePos;
		}

		public static bool IsInsideBoundary(this RectTransform current, Bounds other) =>
			other.Intersects(current.GetBounds());
		...
		public static Vector3 WorldCenter(this RectTransform current)
		{
			...
			for (var i = 1; i < worldCorners.Length; i++) bound.Encapsulate(worldCorners[i]);
