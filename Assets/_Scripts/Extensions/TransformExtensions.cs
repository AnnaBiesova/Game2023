...
namespace _Scripts.Extensions
{
	public static class TransformExtensions
	{
		...
		public static void SetGlobalScale(this Transform transform, Vector3 globalScale)
		{
			...
			helperObject.SetParent(transform.parent);
			transform.localScale = helperObject.localScale;
			helperObject.SetParent(null);
		}
		...
		public static void SetGlobalScale(this Transform transform, float globalScale)
		{
			...
			helperObject.localScale = Vector3.one * globalScale;
		...
		public static void ProcessParallelHierarchies(Transform firstParent, Transform secondParent,
			Action<Transform, Transform> processFunction)
		{
			if (firstParent == null || secondParent == null) return;

			Queue<Transform> firstQueue = new Queue<Transform>();
			Queue<Transform> secondQueue = new Queue<Transform>();

			firstQueue.Enqueue(firstParent);
			secondQueue.Enqueue(secondParent);

			while (firstQueue.Count > 0 && secondQueue.Count > 0)
			{
				Transform firstCurrent = firstQueue.Dequeue();
				Transform secondCurrent = secondQueue.Dequeue();

				processFunction(firstCurrent, secondCurrent);

				// Enqueue children for the next level of traversal
				for (int i = 0; i < firstCurrent.childCount && i < secondCurrent.childCount; i++)
				{
					firstQueue.Enqueue(firstCurrent.GetChild(i));
					secondQueue.Enqueue(secondCurrent.GetChild(i));
				}
			}
		}

		// GETS ONLY ONE COMPONENT FROM OBJECT!
		// Overload that uses a preallocated list for intermediate results
		public static void GetComponentsInChildrenWithDepth<T>(this Transform parent, int depth, List<T> results)
			where T : Component
		{
			if (depth < 0) return;

			results.Clear(); // Clear the preallocated list
			
			SearchChildren(parent, 0);

			// Recursive function to search children
			void SearchChildren(Transform currentTransform, int currentDepth)
			{
				if (currentDepth > depth) return;

				// GETS ONLY ONE COMPONENT FROM OBJECT!
				// Add component from the current transform
				
				if(currentTransform.TryGetComponent(out T comp)) results.Add(comp);
				
				// Iterate through children and search recursively
				foreach (Transform child in currentTransform)
				{
					SearchChildren(child, currentDepth + 1);
				}
			}
		}

		// Original method that returns a new array
		public static T[] GetComponentsInChildrenWithDepth<T>(this Transform parent, int depth) 
			where T : Component
		{
			List<T> results = new List<T>(8);
			parent.GetComponentsInChildrenWithDepth(depth, results);
